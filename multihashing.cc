...
NAN_METHOD(groestlmyraid) {
  ...
  // Hash Input Data and Return Output
  groestlmyraid_hash(input, output, input_len);
  info.GetReturnValue().Set(Nan::CopyBuffer(output, 32).ToLocalChecked());
}
...
NAN_MODULE_INIT(init) {
  ...
  Nan::Set(target, Nan::New("groestlmyraid").ToLocalChecked(), Nan::GetFunction(Nan::New<FunctionTemplate>(groestlmyraid)).ToLocalChecked());
